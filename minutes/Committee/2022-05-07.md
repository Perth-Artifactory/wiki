...
-   Open Source Hackers (Coordinators: Stephen Price*) - NickB has requested a reduction in rate to \$5 (approved) for the event. Also a key for an event host that isn't a member (approved but recipient hasn't been nominated). POSH/PLUG have also enquired about sharing the cost of theMeetup subscription, told them that if they wanted to split ours that would be fine but splitting theirs would require more discussion.
