...
* The workshop lights are now network controllable which was the last item on the workshop automation todo list. Lockup procedure is now:
...
> | Open to current members and those that leave after this decision has been made (based on TidyHQ email) | Same difficulty as invite if lighter registration restriction | Pros | Cons |
> | Open to current members (based on TidyHQ email) | Same difficulty as invite if lighter registration restriction | Pros | Cons |
> | Some combination of the above but only advertised via Slack | Same difficulty as invite if lighter registration restriction | Pros | Cons |
...
| Adopt the permission structure selected above. To take effect immediately. | Lewis       | Phoebe          | Accepted |
