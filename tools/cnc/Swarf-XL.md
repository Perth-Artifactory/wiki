...
- Respiratory Protection: Operate the machine with the dust extraction system enabled whenever possible. You may wish to use a dust mask or respirator rated for woodworking to prevent inhalation of wood dust particles.
...
- Only trained and authorized people should operate the CNC router. Reading these instructions does not constitute training. You can check your authorization status using the “Training Tracker” tool on Slack. 
- Even if you are authorized, seek further instruction if you are unsure about any aspect of machine operation.

## Preparing for Operation
### 1.	Power On: 
Ensure the CNC router and the computer controlling it are powered on by switching on the isolator on the machine and then starting the computer via the button on top and to the right of the monitor.
...
If your job requires more than one tool then you can save some time and hassle by using the prove to measure the length of each tool. If you are going to use this feature, then make sure you measure the length of the first tool before setting the workpiece origin.
...
- Install the dust shoe on the spindle. Ensure that the blast gate for the machine is open and all other blast gates are closed.
...
If the machine is damaged or seems to be operating abnormally, do not attempt to repair it. Report the issue in #cnc-router and place an out-of-service tag on the machine.
