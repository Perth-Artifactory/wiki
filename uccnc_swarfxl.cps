...
longDescription = "Generic post for machines using UCCNC control software. If you are using a fourth axis, make sure to select which axis it is mounted along and the feedrate type (Inverse time or the default, degrees per minute). If you have an automatic tool changer you need to enable the property 'useToolCall'. For laser cutting, set your desired power setting by using property 'Laser power percentage'.";
