...
description = "UCCNC for Swarf XL";
vendor = "UCCNC";
...
longDescription = "UCCNC post-processor modified for Swarf XL. Redundant post options removed; tool changes no longer write an offset from the tool table (as tool changes are manual and tool numbering not used)";
...
function onSection() {
  ...
  if (insertToolCall || retracted) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      if (!jetMode) {
        // writeBlock(gFormat.format(49), hFormat.format(lengthOffset)); BC: don't want write or clear tool offsets
      }
      writeBlock(
      ...
      if (!jetMode) {
        writeBlock(gFormat.format(49), hFormat.format(lengthOffset));
      }
      writeBlock(
        gAbsIncModal.format(90),
        gMotionModal.format(0),
